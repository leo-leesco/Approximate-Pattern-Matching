...
int levenshtein(char *s1, char *s2, int len, int *column) {
  ...
  for (x = 1; x <= len; x++) {
    ...
    for (y = 1; y <= len; y++) {
      olddiag = column_y;
      column_y = MIN3(column_y + 1, column_y_minus_1 + 1,
...
__global__ void gpu_pattern_matching(const char *text, int text_length,
                                     int chunk_size, const char *patterns,
                                     int *match_counts, int size_pattern,
                                     int approx_factor, int pattern_idx) {
...
void cuda_pattern_matching(const char *text, int text_length, int chunk_size,
                           char **patterns, int num_patterns, int approx_factor,
                           int *match_counts) {
  ...
  for (int i = 0; i < num_patterns; i++) {
    ...
    cudaMemcpy(d_patterns + offset, patterns[i], pattern_length * sizeof(char),
...
#pragma omp parallel for schedule(static)
  for (int i = 0; i < num_patterns; i++) {
    ...
    gpu_pattern_matching<<<gridSize, blockSize>>>(
        d_text, text_length, chunk_size, d_patterns + offsets[i],
        d_match_counts, pattern_length, approx_factor, i);
  }

  // Synchronize to ensure all GPU work is completed before copying results
  cudaDeviceSynchronize();

  cudaMemcpy(match_counts, d_match_counts, num_patterns * sizeof(int),
...
int main(int argc, char **argv) {
  ...
  cuda_pattern_matching(local_buf, local_size, chunk_size, pattern, nb_patterns,
                        approx_factor, n_matches);
  ...
  MPI_Reduce(n_matches, global_matches, nb_patterns, MPI_INT, MPI_SUM, 0,
             MPI_COMM_WORLD);
